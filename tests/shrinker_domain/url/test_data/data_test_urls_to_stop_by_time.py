...
test_data = [
    ...
    test_data_generator(
        description="Негативная проверка: URL_CISCO, протоколы: all, JSON, to STOP by time",
        out_type="JSON",
        filter_urls=urls_lists[0],
        filter_range=[
            int((datetime.datetime.now(timezone) - datetime.timedelta(hours=random.randint(1, 12))).timestamp()) * 1000,
            int((datetime.datetime.now(timezone) + datetime.timedelta(seconds=100)).timestamp()) * 1000,
        ],
